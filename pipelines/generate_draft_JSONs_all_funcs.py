...
def add_basset_sad_sat(G, homo_gen, target_node_name,out_dir):
    ...
    with open(targets_file) as id2type:
        for c_type in id2type:
            ...
            count += 1
    cmd = ("basset_sad.py -f " + homo_gen.filename + " -l 600 -o " + basset_out_name + " -t " + targets_file +
    ...
    cmd = ("basset_sat_vcf.py -f " + homo_gen.filename + " -t " + str(top_snp_ctype_target) + " -o " + basset_out_name + 
           " " + model_file + " " + basset_out_name + "/top_variant.vcf")
    os.system(cmd)
    #Save output pdfs & cleanup
    #tag in JSON file too
    G.node[top_snp_names[0]]["sad_pdf"] = target_in_name + "_sad_heat.pdf"
    out_pdfs = [pdf for pdf in glob.glob(basset_out_name+"/*heat.pdf") if target_sad_name in pdf]
    label_snp = ["ref","alt"]
    count = 1
    for pdf in out_pdfs:
        pdf_sat = pdf.split("/")
...
def basset_predict_tf_range(open_region, homo_gen, target_node_name, openpeak2bassetpeak):
    ...
    with open("basset_open_anchor.fa", "w+") as open_seqs_fa:
        SeqIO.write(open_seqs, open_seqs_fa, "fasta")
    cmd = ("seq_hdf5.py -r -c -v " + str(len(open_seqs)) + " -t " + str(len(open_seqs)) 
           + " basset_open_anchor.fa /input_data/encode_roadmap_act.txt open_region.h5")
    subprocess.call(cmd, shell=True)
    cmd = ("basset_motifs.py -s " + str(len(open_seqs)) + " -t -o motifs_out + " 
...
if __name__ == "__main__":
    ...
    if cell_type == "primed":
        ...
        dna_ints = home_dir + "primed_n2.bedpe" #Primed-SMC1-petcount-filtered.bedpe,Primed-SMC1-petcount.bedpe, primed_n2.bedpe
    ...
    with open(dna_ints) as dna_ints_iter:
        for dna_int in dna_ints_iter:
            ...
            if (arr[0] in chr_names) and (arr[3] in chr_names):
                dna_int_graph.add_edge(x,y,label=1, capacity = 1, weight=int(float(arr[6])))
